...
function validateCandidateNarrativePath(candidate = {}) {
  const canonicalGate = candidate?.pipeline_contract?.narrative_path_validation
    || candidate?.pipelineContract?.narrative_path_validation
    || null;

  if (canonicalGate && typeof canonicalGate.approved === 'boolean') {
  ...
  if (!validation) {
    return { approved: false, status: 'NTG_PATH_MISSING', reason: 'Candidate não possui validação de Narrative Path.' };
  }

  const incompatible = Boolean(validation.blocking_incompatibility || validation.incompatible_pair_count > 0 || candidate.ntg_incompatible);
  const completeEnough = validation.complete_enough !== false && Number(validation.complete_path_count || 0) >= 2;
  const coherent = validation.coherent === true || (!incompatible && completeEnough && Number(validation.average_graph_score || candidate.graph_narrative_score || 0) >= 45);
  ...
  return {
    ...
    graph_score: Number(validation.average_graph_score || candidate.graph_narrative_score || 0),
    validation,
  };
}
...
export class StoryTruthPipeline {
  ...
  async execute(context) {
    ...
    const narrativePathGate = context.narrativePathValidation?.approved !== undefined
      ? context.narrativePathValidation
      : validateCandidateNarrativePath(candidateForSelection);
    context.narrativePathValidation = narrativePathGate;
    context.pipelineContract = {
      ...(context.pipelineContract || context.candidate?.pipeline_contract || {}),
      narrative_path_validation: narrativePathGate,
      truth_pipeline: {
        status: narrativePathGate.approved ? 'AUTHORIZED' : 'BLOCKED',
        keep_only: true,
        executed_at: new Date().toISOString(),
      },
    };

    if (!narrativePathGate.approved) {
      context.addWarning('Narrative Path NTG bloqueou a geração da Story.', {
        status: narrativePathGate.status,
        graph_score: narrativePathGate.graph_score,
        incompatible: narrativePathGate.incompatible,
      });
      context.truthSelection = {
        selected: [], used_memories: [], optional: [], discarded: context.memories || [],
        statistics: { total: (context.memories || []).length, keep: 0, optional: 0, drop: (context.memories || []).length },
        ntg_gate: narrativePathGate,
        source_policy: 'NTG é critério principal. Candidate incompatível ou sem caminho completo não chega à geração editorial.',
      };
      return context;
    }
    ...
    context.truthReport = {
      ...
      truth_prompt: {
        ...
        ntg_primary: true,
        embeddings_auxiliary_only: true,
        source_policy: "Story Truth Pipeline valida primeiro o Narrative Path NTG e envia ao Truth Domain apenas memórias KEEP selecionadas e auditáveis.",
      },
    };
